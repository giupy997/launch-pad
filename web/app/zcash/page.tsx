...
import { CURVE_SUPPLY, spotPrice } from "@/lib/zcash/ledger";
import { fmtPrice, fmtZec, shortKey, useZcashState, type ZCoin } from "@/lib/zcash/client";
import { TokenLogo } from "@/components/TokenLogo";

export default function ZcashExplore() {
    ...
    <div className="space-y-10">
      <section className="text-center space-y-4 py-6">
        ...
        <p className="text-zinc-400 max-w-xl mx-auto">
          Zcash has no smart contracts — so a coin here begins as an encrypted memo. One shielded
          address, a public viewing key, and a bonding curve anyone can recompute from the chain.
        </p>
        <Link
          href="/zcash/create"
      ...
      <section className="grid grid-cols-2 sm:grid-cols-4 gap-3">
        ...
        <Stat label="Memos read" value={String(state?.memosRead ?? 0)} />
        <Stat
          label="State root"
...
function CoinCard({ coin: c }: { coin: ZCoin }) {
  ...
  return (
    ...
    >
      ...
      <div className="mt-3 text-xs text-zinc-500">creator {shortKey(c.creator)}</div>
